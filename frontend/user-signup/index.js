async function validerEmail() {
            ...
            if (!emailInput.toLowerCase().endsWith(tilladtDomæne)) {
                beskedElement.textContent = "Fejl: Du skal bruge en " + tilladtDomæne + " adresse.";
                return;
            }
            ...
            const res = await fetch("/api/auth/create-user", {
             ...
             if (!res.ok) {
                ...
                beskedElement.textContent = "Fejl: " + (errorData.message || errorData.error || errorData || "Ukendt fejl opstod.");
                return;
            }

            setTimeout(function() {
                window.location.href = "confirm.html" + "?email=" + encodeURIComponent(emailInput);
            }, 0);
        }
        function validerAdgangskode() {
            const adgangskode = document.getElementById("adgangskodeFelt").value;
            const bekraftAdgangskode = document.getElementById("bekraftAdgangskodeFelt").value;
            const beskedElement = document.getElementById("beskedFelt");
